...
#if 0
/* the routine below is not really needed with gcc, because gcc has long long.
    gcc uses the fpu which introduces a few extra instructions. Someone has
    complained about compile problems with this, so I'm leaving it out. */
...
void TD_draw_solid (TD_Solid * s)
{
	...
	fprintf (stderr, "%s: TD_draw_solid(): Error allocating memory.\n", PACKAGE);
	goto fin;
    }
	...
	for (j = 0, i = 0; i < n; i++) {
		...
		hw = s->surf[i].w;
		hl = s->surf[i].l;
/* must test if width or length are even or odd */
		if (!(hw & 1) && !(hl & 1)) {
		    distance = TD_finddistance (s, &s->surf[i].point[hw / 2
						       + hw * (hl / 2)]);
		    distance += TD_finddistance (s, &s->surf[i].point[hw / 2 - 1
						       + hw * (hl / 2)]);
		    distance += TD_finddistance (s, &s->surf[i].point[hw / 2
						   + hw * (hl / 2 - 1)]);
		    distance += TD_finddistance (s, &s->surf[i].point[hw / 2 - 1
