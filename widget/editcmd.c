...
{E_
    ...
    if (!old_pattern || strcmp (old_pattern, pattern) || old_type != match_type || old_icase != icase || old_utf8 != cooledit_regex_get_utf8_or_ascii()) {
	if (old_pattern) {
	    regfree (&r);
	    free (old_pattern);
	    old_pattern = 0;
	...
	old_type = match_type;
	old_icase = icase;
	old_utf8 = cooledit_regex_get_utf8_or_ascii();
    }
