...
typedef char *mime_type_list_t[NUM_SIMPLE_TYPES][10];

/* each entry is listed in order of what we would like the RECEIVE */
static mime_type_list_t mime_type_send =
{
    {0},			/* DndUnknown 0 */
    {0},			/* DndRawData 1 */
...
static mime_type_list_t mime_type_recieve =
{
    {0},	/* DndUnknown 0 */
    {0},	/* DndRawData 1 */
...
/* each entry is listed in order of what we would like the RECEIVE */
static mime_type_list_t mime_type_send_utf8 =
{
    {0},			/* DndUnknown 0 */
    {0},			/* DndRawData 1 */
    {"url/url", "text/plain;charset=utf-8", "text/plain;charset=UTF-8", 0},	/* DndFile    2 (input widget) */
    {"text/plain;charset=utf-8", "text/plain;charset=UTF-8", "text/uri-list", 0},	/* DndFiles   3 (fielded text box) */
    {"text/plain;charset=utf-8", "text/plain;charset=UTF-8", "application/octet-stream", 0},	/* DndText    4 (editor widget) */
/* don't use these: */
    {0},			/* DndLink    5 */
    {0},			/* DndDir     6 */
    {0},			/* DndExe     7 */
...
static mime_type_list_t mime_type_recieve_utf8 =
{
    {0},	/* DndUnknown 0 */
    {0},	/* DndRawData 1 */
    {"url/url", "text/uri-list", "text/plain;charset=utf-8", "text/plain;charset=UTF-8", 0},	/* DndFile    2 (input widget) */
    {0},	/* DndFiles   3 */
    {"url/url", "text/plain;charset=utf-8", "text/plain;charset=UTF-8", "application/octet-stream", "text/uri-list", 0},	/* DndText    4 (editor widget) */
/* don't use these: */
    {0},			/* DndLink    5 */
    {0},			/* DndDir     6 */
    {0},			/* DndExe     7 */
...
/*
   Takes a newline separate list of files,
   returns a newline separate list of complete `file:' path names
...
/* doesn't have mouse funcs - not a drop widget?? */
    if (!w->funcs)
        return 0;
...
/* look up mime types from my own list of supported types */
	...
	memset (names_return, 0, (array_length (typelist) + 1) * sizeof (char *));
	if (my_XGetAtomNames (CDisplay, typelist, array_length (typelist), names_return)) {
...
/* not supported, so return false */
    ...
    if (xd < -40 || yd < -40 || xd >= CWidthOf (w) + 40 || yd >= CHeightOf (w) + 40)
	return 0;

/* things that can receive a drop are in the editor font by definition. */
    CPushFont ("editor", 0);
...
static void xdnd_set_dnd_mime_types_ (mime_type_list_t mt_send, mime_type_list_t mt_recv)
{
    ...
    for (i = 0; i < NUM_SIMPLE_TYPES; i++) {
        ...
        for (j = 0; mt_recv[i][j]; j++) {
            xdnd_typelist_receive[i][j] = XInternAtom (CDndClass->display, mt_recv[i][j], False);
            xdnd_typelist_receive[i][j + 1] = 0;
        }
        xdnd_typelist_receive[i + 1] = 0;
        xdnd_typelist_send[i] = CMalloc (32 * sizeof (Atom));
        for (j = 0; mt_send[i][j]; j++) {
            xdnd_typelist_send[i][j] = XInternAtom (CDndClass->display, mt_send[i][j], False);
...
void xdnd_set_dnd_mime_types (int utf8)
{
    if (utf8)
        ...
        xdnd_set_dnd_mime_types_ (mime_type_send, mime_type_recieve);
}
...
void mouse_mark (XEvent * event, int double_click, struct mouse_funcs *funcs)
{
	...
	if (!(event->xmotion.state & Button12345Mask))
	    return;
	(*funcs->xy) (event->xmotion.x, event->xmotion.y, &x, &y);
