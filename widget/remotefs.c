...
{E_
    assert (io->cmd_fd == -1);
    assert (!io->remotefs);
    assert (!strcmp (io->host, host));
    io->remotefs = remotefs_new (host, errmsg);
    if (!io->remotefs)
        return -1;
    if ((*io->remotefs->remotefs_shellreconnect) (io->remotefs, io, errmsg)) {
...
{E_
  ...
  retry:
    ...
    if (r > 0) {
        ...
        *error_code = RFSERR_SERVER_CLOSED_SHELL_DIED;
        if (io->cmd_pid) {
            int wstatus = 0;
            for (;;) {
                childhandler_ ();
...
static int local_shellkill (struct remotefs *rfs, struct remotefs_terminalio *io)
{E_
    if (lookup_cterminal (io->cmd_pid)) {
        kill (io->cmd_pid, SIGTERM);
        delete_cterminal (io->cmd_pid);
    }
    return 0;
}
...
void remotefs_suspend_terminalio (struct remotefs_terminalio *io)
{E_
    if (io->remotefs) {
        remotefs_free (io->remotefs);
        io->remotefs = NULL;
        io->reader_data->sock_data = NULL;
    }
}
...
static int terminal_undead_createpath (char *path, int pathsz, const char *hostip, const struct cterminal_config *config)
{
    ...
    mkdir (dir, 0700);
    snprintf (path, pathsz, "%s/%lu", dir, config->cmd_pid);
    return 0;
}
...
static int remote_shellreconnect (struct remotefs *rfs, struct remotefs_terminalio *io, char *errmsg)
{E_
    CStr s, msg;
    unsigned char *q;
    int no_such_action = 0;
...
static int remote_shellkill (struct remotefs *rfs, struct remotefs_terminalio *io)
{
    unsigned char *q;
    CStr msg;
...
static int dummyerr_shellreconnect (struct remotefs *rfs, struct remotefs_terminalio *io, char *errmsg)
{E_
    return remotefs_error_return (errmsg);
}
...
static int dummyerr_shellkill (struct remotefs *rfs, struct remotefs_terminalio *io)
{E_
    return -1;
}
...
static int remote_action_fn_v5_shellreconnect (struct server_data *sd, CStr *s, const unsigned char *in, int inlen)
{E_
    const unsigned char *p, *end;
