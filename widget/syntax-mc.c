...
static void syntax_keyword_free (gpointer keyword)
{
...
static void context_rule_free (gpointer rule)
{
...
static gint mc_defines_destroy (gpointer key, gpointer value, gpointer data)
{
    (void) data;
...
inline static int xx_tolower (const WEdit * edit, int c)
{
...
static void subst_defines (GTree * defines, char **argv, char **argv_end)
{
...
static off_t
compare_word_to_right (const WEdit * edit, off_t i, const char *text, const char *whole_left, const char *whole_right, gboolean line_start)
{
    ...
    return (whole_right != NULL && strchr (whole_right, xx_tolower (edit, edit_buffer_get_byte (&edit->buffer, i))) != NULL) ? -1 : i;
}
...
static void apply_rules_going_right (WEdit * edit, off_t i)
{
    context_rule_t *r;
    int c;
	...
	if (!found_left && is_end) {
		...
		if (r->between_delimiters) {
			...
			e = compare_word_to_right (edit, i, r->right, r->whole_word_chars_left, r->whole_word_chars_right, r->line_start_right);
			if (e >= end) {
			    _rule.end = e;
			    found_right = TRUE;
			    _rule.border = RULE_ON_RIGHT_BORDER;
...
static void edit_get_rule (WEdit * edit, off_t byte_index)
{
    off_t i;
...
static int translate_rule_to_color (const WEdit * edit, const edit_syntax_rule_t * rule)
{
    syntax_keyword_t *k;
    context_rule_t *r;
...
static size_t read_one_line (char **line, FILE * f)
{
    GString *p;
    size_t r = 0;
...
static int this_try_alloc_color_pair (const char *fg, const char *bg, const char *attrs)
{
...
static FILE *open_include_file (const char *filename)
{
    ...
    g_free (error_file_name);
    error_file_name = g_build_filename (mc_config_get_data_path (), EDIT_HOME_DIR, filename, (char *) NULL);
...
static int edit_read_syntax_rules (WEdit * edit, FILE * f, char **args, int args_size)
{
    FILE *g = NULL;
    char *fg, *bg, *attrs;
...
static int
edit_read_syntax_file (WEdit * edit, GPtrArray * pnames, const char *syntax_file, const char *editor_file, const char *first_line, const char *type)
{
    FILE *f, *g = NULL;
...
static const char *get_first_editor_line (WEdit * edit)
{
    static char s[256];
...
static int exec_edit_syntax_dialog (const GPtrArray * names, const char *current_syntax)
{
    size_t i;
    Listbox *syntaxlist;

    syntaxlist = create_listbox_window (LIST_LINES, MAX_ENTRY_LEN, _("Choose syntax highlighting"), NULL);
...
void edit_load_syntax (WEdit * edit, GPtrArray * pnames, const char *type)
{
	...
	r = edit_read_syntax_file (edit, pnames, f, vfs_path_as_str (edit->filename_vpath),
				   get_first_editor_line (edit), option_auto_syntax ? NULL : edit->syntax_type);
    else
	...
	edit_free_syntax_rules (edit);
	message (D_ERROR, _("Load syntax file"), _("Error in file %s on line %d"), error_file_name != NULL ? error_file_name : f, r);
	MC_PTR_FREE (error_file_name);
    }
...
void edit_syntax_dialog (WEdit * edit)
{
	...
	if (force_reload || (option_auto_syntax && !old_auto_syntax) || old_auto_syntax ||
	    (current_syntax != NULL && edit->syntax_type != NULL && strcmp (current_syntax, edit->syntax_type) != 0))
